...
test('home to upload thumbnails to editor flow works', async ({ page }) => {
  ...
  await expect(page.getByText('EN · Going to work at Daiso')).toBeVisible()
  ...
  await page.getByLabel('1번 클립 문구').fill('머리핀을 꽂았어요❤️')
  await expect(page.getByText(/EN · I put on a hair clip/)).toBeVisible()

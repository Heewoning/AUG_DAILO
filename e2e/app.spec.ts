...
test('home to upload thumbnails to editor flow works', async ({ page }) => {
  ...
  await expect(page.locator('.xp-english-caption')).toHaveCount(0)
  await page.getByLabel('영어 자막').blur()
  await page.getByRole('button', { name: '자동 번역' }).click()
  await expect(page.getByLabel('영어 자막')).toHaveValue(/I put on a hair clip/)
  await expect(page.locator('.xp-english-caption')).toBeVisible()
  ...
  await page.getByLabel('말풍선 유지 시간').fill('0.4')
  await page.getByLabel('다음 컷 체력').fill('35')
  await page.getByLabel('말풍선 유지 시간').blur()
  await expect(page.locator('.xp-vlog-widget--energy-bar')).toBeVisible()
  await expect(page.locator('.xp-vlog-widget--energy-bar')).toContainText('35% · DRAINING')
  await expect(page.locator('.phone-preview')).toHaveClass(/phone-preview--intermission/)

...
test('home to upload thumbnails to editor flow works', async ({ page }) => {
  ...
  const selectedSource = await page.getByLabel('morning-coffee.webm 미리보기').evaluate((video: HTMLVideoElement) => video.currentSrc)
  ...
  await expect.poll(() => page.locator('.phone-preview video').evaluate((video: HTMLVideoElement) => video.currentSrc)).toBe(selectedSource)
